...
void Game::InitView(void){
	...
	camera_.SetThirdPerson(false);
	camera_.SetMoved(false);
}
...
void Game::MainLoop(void){
	...
	while (!glfwWindowShouldClose(window_)) {
		...
		glm::vec3 thirdPersonVec = glm::vec3(0.0, 0.0, 10.0);
		if (camera_.GetThirdPerson()){
			if (!camera_.GetMoved()) {
				camera_.SetPosition(camera_.GetPosition() - thirdPersonVec);
				camera_.SetMoved(true);
			}
		}
		else {
			if (camera_.GetMoved()) {
